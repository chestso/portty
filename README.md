...
- Lottie animations — APC sequences (`ESC _ … ST`) with base64-encoded JSON payloads load, place, and control Lottie animations on the grid. Eight commands (load, load-chunk, place, play, pause, stop, seek, delete) manage animation state and placement tracking. Animations scroll with the text, enter scrollback, and are cleared with the rows they sit on — the same ownership model as sixel. The host fetches RGBA frames via the bloom-vt API (`bvt_get_lotties()`, `bvt_get_lottie_placements()`, `bvt_lottie_tick()`) and composites them as an overlay layer. Rasterization (ThorVG) is not yet wired; RGBA buffers are currently zeroed
